...
def copyDir(srcDirectory, dstDirectory, productName):
    ...
    for item in os.listdir(srcDirectory):
        ...
        if os.path.isfile(src_path):
            ...
            f_dst.write(data)
            f_dst.close()
        elif os.path.isdir(src_path) and not '.svn' in src_path:
...
def makeNewProduct(
    ...
    # Form CSS and JS importing list and regestry data (looking in
    # subdir too) for Plone 2.1.0+

    stylesheets_xml = ''
    javascripts_xml = ''

    # subdir = subdir or getToolByName(context,
    # 'portal_skins')[zmi_skin_name]

    portal_setup = getToolByName(context, 'portal_setup')
    result_css_list = skin_css_regdata = result_js_list = \
        skin_js_regdata = []
    ...
    if dump_CSS:
        ...
        # result_css_list, skin_css_regdata =
        # getResourceListRegdata(context, subdir, CSS_PATTERN,
        # 'portal_css', CSS_REG_PROPS)

    if stylesheets_xml == '':
        files_to_remove.append(ospJoin('profiles', 'default',
                               'cssregistry.xml'))
    if dump_JS:
        ...
        # result_js_list, skin_js_regdata =
        # getResourceListRegdata(context, subdir, JS_PATTERN,
        # 'portal_javascripts', JS_REG_PROPS)
    ...
    slots = ()
    if dump_portlets:
        slots = dumpPortlets(context, dump_policy,
                             dump_portlets_selection)
    ...
    for profile in profiles:
        ...
        update_dict = dict(fname=file_name,
                           fpath=ospJoin(profiles_path, profile,
                           file_name),
                           fdata='# Marker file for %s profile of %s skin'
                            % (profile, productName))
        locals()[varname].update(update_dict)
    ...
    conf_dict = {
        ...
        'skin_name_capital': '%s%s' % (productSkinName[0].upper(),
                productSkinName[1:]),
        ...
        'directory_views_xml': fsDirectoryViewsXML(zmi_skin_names,
                productName),
        'remove_directory_views_xml': fsDirectoryViewsXML(zmi_skin_names,
                productName, remove=True),
    ...
    sp_updated_files = [
        ...
        ospJoin('browser', 'configure.zcml'),
                       ...
                       # ,ospJoin('profiles', 'uninstall',
                       # 'cssregistry.xml')\ ,ospJoin('profiles',
                       # 'uninstall', 'jsregistry.xml')\
